...
t_max_err ears_buffer_retain(t_buffer_obj *buffer, t_symbol *buffername, t_llll *generated_names, bool dynamic_mode)
{
    t_max_err err = MAX_ERR_NONE;
    
    // WARNING: THIS LINE WAS: Restore if there are issues! – but the new line should be ok
//    if (generated_names && is_symbol_in_llll_first_level(generated_names, buffername)) {
    if (dynamic_mode && generated_names && is_symbol_in_llll_first_level(generated_names, buffername)) {
...
t_ears_err ears_buffer_onepole_envelope(t_object *ob, t_buffer_obj *source, t_buffer_obj *dest, t_llll *cutoff_freq, char highpass, e_slope_mapping slopemapping)
{
    ...
    if (!orig_sample) {
        ...
        if (!dest_sample) {
            ...
            for (long f = 0; f < framecount; f++) {
                double cutoff_freq = ears_envelope_iterator_walk_interp(&eei, f, framecount);
                b1 = exp(-2.0 * PI * (cutoff_freq/sr));
                a0 = 1.0 - b1;
                for (long c = 0; c < channelcount; c++) {
                    idx = f * channelcount + c;
