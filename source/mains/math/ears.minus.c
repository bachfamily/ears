/**
	...
	@digest
	Subtract buffer samples
 
	@description
	Subtracts a given number, envelope or buffer from buffer samples.
...
// Prototypes
t_buf_minus*         buf_minus_new(t_symbol *s, short argc, t_atom *argv);
void			buf_minus_free(t_buf_minus *x);
void			buf_minus_bang(t_buf_minus *x);
void			buf_minus_anything(t_buf_minus *x, t_symbol *msg, long ac, t_atom *av);

void buf_minus_assist(t_buf_minus *x, void *b, long m, long a, char *s);
void buf_minus_inletinfo(t_buf_minus *x, void *b, long a, char *t);
...
EARSBUFOBJ_ADD_IO_METHODS(minus)
...
void C74_EXPORT ext_main(void* moduleRef)
{
    ...
    CLASS_NEW_CHECK_SIZE(c, "ears.minus~",
                         (method)buf_minus_new,
                         (method)buf_minus_free,
                         sizeof(t_buf_minus),
                         (method)NULL,
                         A_GIMME,
                         0L);
    ...
    EARSBUFOBJ_DECLARE_COMMON_METHODS_HANDLETHREAD(minus)
    
    // @method number @digest Set operand
    // @description A number in the second inlet sets a constant second operand

    
    CLASS_ATTR_CHAR(c, "scalarmode",    0,    t_buf_minus, e_scalarmode);
...
t_buf_minus *buf_minus_new(t_symbol *s, short argc, t_atom *argv)
{
    ...
    x = (t_buf_minus*)object_alloc_debug(s_tag_class);
    if (x) {
...
void buf_minus_bang(t_buf_minus *x)
{
    ...
    if (num_buffers1 > 0 && num_buffers2 > 0) {
        ...
        for (long count = 0; count < numoutbuffers; count++) {
            ...
            ears_buffer_op((t_object *)x, in1, in2, out, EARS_OP_MINUS, (e_ears_resamplingpolicy) x->e_ob.l_resamplingpolicy, x->e_ob.l_resamplingfilterwidth, (e_ears_resamplingmode)x->e_ob.l_resamplingmode);
        ...
        if (x->e_operand->l_size > 0) {
            ...
            for (long count = 0; count < numoutbuffers; count++, el = (el && el->l_next ? el->l_next : el)) {
                ...
                if (el && is_hatom_number(&el->l_hatom)) {
                    ears_buffer_number_op((t_object *)x, in1, hatom_getdouble(&el->l_hatom), out, EARS_OP_MINUS);
                } else if (el && hatom_gettype(&el->l_hatom) == H_LLLL){
                    t_llll *env = earsbufobj_llllelem_to_env_samples((t_earsbufobj *)x, el, in1);
                    ears_buffer_envelope_op((t_object *)x, in1, env, out, EARS_OP_MINUS, (e_slope_mapping) x->e_ob.l_slopemapping);
                    llll_free(env);
                } else {
