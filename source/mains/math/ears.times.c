...
void buf_times_bang(t_buf_times *x)
{
    ...
    if (num_buffers1 > 0 && num_buffers2 > 0) {
        ...
        for (long count = 0; count < numoutbuffers; count++) {
            ...
            ears_buffer_op((t_object *)x, in1, in2, out, EARS_OP_TIMES, (e_ears_resamplingpolicy) x->e_ob.l_resamplingpolicy, x->e_ob.l_resamplingfilterwidth, (e_ears_resamplingmode)x->e_ob.l_resamplingmode);
