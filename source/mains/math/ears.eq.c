/**
	...
	@keywords
	buffer, equal, condition, compare
...
// Prototypes
t_buf_eq*     buf_eq_new(t_symbol *s, short argc, t_atom *argv);
void			buf_eq_free(t_buf_eq *x);
void			buf_eq_bang(t_buf_eq *x);
void			buf_eq_anything(t_buf_eq *x, t_symbol *msg, long ac, t_atom *av);

void buf_eq_assist(t_buf_eq *x, void *b, long m, long a, char *s);
void buf_eq_inletinfo(t_buf_eq *x, void *b, long a, char *t);
...
void C74_EXPORT ext_main(void* moduleRef)
{
    ...
    CLASS_NEW_CHECK_SIZE(c, "ears.eq~",
                         (method)buf_eq_new,
                         (method)buf_eq_free,
                         sizeof(t_buf_eq),
                         (method)NULL,
    ...
    // If the inlet is the first one, then the comparison result is output: 1 if the buffers coincide
    // in all their properties (including the spectral ones, for spectral buffers) and in all their samples, and 0 otherwise,
    EARSBUFOBJ_DECLARE_COMMON_METHODS_HANDLETHREAD(eq)
...
void buf_eq_assist(t_buf_eq *x, void *b, long m, long a, char *s)
{
    if (m == ASSIST_INLET) {
        ...
        sprintf(s, "int: 1 if Buffers Are Equal, 0 Otherwise"); // @out 0 @type int @digest 1 if Buffers Are Equal, 0 Otherwise
    }
}
...
t_buf_eq *buf_eq_new(t_symbol *s, short argc, t_atom *argv)
{
    ...
    x = (t_buf_eq*)object_alloc_debug(s_tag_class);
    if (x) {
...
void buf_eq_bang(t_buf_eq *x)
{
    ...
    long res = 0;
    ears_buffer_eq((t_object *)x, buf1, buf2, &res);
