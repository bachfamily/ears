/**
	@file
	ears.model.peanos.analysis.c
 
	@name
	ears.model.peanos.analysis~
...
typedef struct _buf_model_peanos_analysis {
    t_earsbufobj                e_ob;
    ...
    long    e_downmix;
} t_buf_model_peanos_analysis;



// Prototypes
t_buf_model_peanos_analysis*     buf_model_peanos_analysis_new(t_symbol *s, short argc, t_atom *argv);
void			buf_model_peanos_analysis_free(t_buf_model_peanos_analysis *x);
void			buf_model_peanos_analysis_bang(t_buf_model_peanos_analysis *x);
void			buf_model_peanos_analysis_anything(t_buf_model_peanos_analysis *x, t_symbol *msg, long ac, t_atom *av);

void buf_model_peanos_analysis_assist(t_buf_model_peanos_analysis *x, void *b, long m, long a, char *s);
void buf_model_peanos_analysis_inletinfo(t_buf_model_peanos_analysis *x, void *b, long a, char *t);


// Globals and Statics
static t_class	*s_model_peanos_analysis_class = NULL;
static t_symbol	*ps_event = NULL;

EARSBUFOBJ_ADD_IO_METHODS(model_peanos_analysis)
...
void C74_EXPORT ext_main(void* moduleRef)
{
    ...
    CLASS_NEW_CHECK_SIZE(c, "ears.model.peanos.analysis~",
                         (method)buf_model_peanos_analysis_new,
                         (method)buf_model_peanos_analysis_free,
                         sizeof(t_buf_model_peanos_analysis),
                         (method)NULL,
                         A_GIMME,
                         0L);
    ...
    EARSBUFOBJ_DECLARE_COMMON_METHODS_HANDLETHREAD(model_peanos_analysis)
    ...
    CLASS_ATTR_DOUBLE(c, "freqdevoffset", 0, t_buf_model_peanos_analysis, e_freqDevOffset);
    ...
    CLASS_ATTR_DOUBLE(c, "freqdevslope", 0, t_buf_model_peanos_analysis, e_freqDevSlope);
    ...
    CLASS_ATTR_DOUBLE(c, "magnitudethreshold", 0, t_buf_model_peanos_analysis, e_magnitudeThreshold);
    ...
    CLASS_ATTR_DOUBLE(c, "minfrequency", 0, t_buf_model_peanos_analysis, e_minFrequency);
    ...
    CLASS_ATTR_DOUBLE(c, "maxfrequency", 0, t_buf_model_peanos_analysis, e_maxFrequency);
    ...
    CLASS_ATTR_LONG(c, "maxpeaks", 0, t_buf_model_peanos_analysis, e_maxPeaks);
    ...
    CLASS_ATTR_SYM(c, "orderby", 0, t_buf_model_peanos_analysis, e_orderBy);
    ...
    CLASS_ATTR_LONG(c, "maxnpeanoss", 0, t_buf_model_peanos_analysis, e_maxnpeanoss);
    CLASS_ATTR_STYLE_LABEL(c,"maxnpeanoss",0,"text","Maximum Number of peanoss per Frame");
    CLASS_ATTR_CATEGORY(c, "maxnpeanoss", 0, "Analysis");
    // @description Sets the maximum number of peanoss per frame.

    CLASS_ATTR_LONG(c, "downmix",    0,    t_buf_model_peanos_analysis, e_downmix);
    ...
    s_model_peanos_analysis_class = c;
...
t_buf_model_peanos_analysis *buf_model_peanos_analysis_new(t_symbol *s, short argc, t_atom *argv)
{
    t_buf_model_peanos_analysis *x;
    long true_ac = attr_args_offset(argc, argv);
    
    x = (t_buf_model_peanos_analysis*)object_alloc_debug(s_model_peanos_analysis_class);
    if (x) {
...
void buf_model_peanos_analysis_bang(t_buf_model_peanos_analysis *x)
{
    ...
    if (num_buffers >= 1) {
        ...
        for (long i = 0; i < num_channels; i++) {
            ...
            t_ears_err err = ears_model_peanos_analysis((t_object *)x, samps[i], sr,
    ...
    if (parsed && parsed->l_head) {
        if (hatom_gettype(&parsed->l_head->l_hatom) == H_SYM) {
            if (inlet == 0) {
                ...
                buf_model_peanos_analysis_bang(x);
            }
        }
    }
    llll_free(parsed);
