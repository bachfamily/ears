/**
	...
	@digest
	Sinusoidal-plus-stochastic analysis model
 
	@description
	Applies the Essentia sinusoidal-plus-stochastic analysis model
...
typedef struct _buf_model_sps_analysis {
    t_earsbufobj                e_ob;
    ...
    long    e_downmix;
} t_buf_model_sps_analysis;



// Prototypes
t_buf_model_sps_analysis*     buf_model_sps_analysis_new(t_symbol *s, short argc, t_atom *argv);
void			buf_model_sps_analysis_free(t_buf_model_sps_analysis *x);
void			buf_model_sps_analysis_bang(t_buf_model_sps_analysis *x);
void			buf_model_sps_analysis_anything(t_buf_model_sps_analysis *x, t_symbol *msg, long ac, t_atom *av);

void buf_model_sps_analysis_assist(t_buf_model_sps_analysis *x, void *b, long m, long a, char *s);
void buf_model_sps_analysis_inletinfo(t_buf_model_sps_analysis *x, void *b, long a, char *t);


// Globals and Statics
static t_class	*s_model_sps_analysis_class = NULL;
static t_symbol	*ps_event = NULL;

EARSBUFOBJ_ADD_IO_METHODS(model_sps_analysis)
...
void C74_EXPORT ext_main(void* moduleRef)
{
    ...
    CLASS_NEW_CHECK_SIZE(c, "ears.model.sps.analysis~",
                         (method)buf_model_sps_analysis_new,
                         (method)buf_model_sps_analysis_free,
                         sizeof(t_buf_model_sps_analysis),
                         (method)NULL,
                         A_GIMME,
                         0L);
    ...
    EARSBUFOBJ_DECLARE_COMMON_METHODS_HANDLETHREAD(model_sps_analysis)
    ...
    CLASS_ATTR_DOUBLE(c, "freqdevoffset", 0, t_buf_model_sps_analysis, e_freqDevOffset);
    ...
    CLASS_ATTR_DOUBLE(c, "freqdevslope", 0, t_buf_model_sps_analysis, e_freqDevSlope);
    ...
    CLASS_ATTR_DOUBLE(c, "magnitudethreshold", 0, t_buf_model_sps_analysis, e_magnitudeThreshold);
    ...
    CLASS_ATTR_DOUBLE(c, "minfrequency", 0, t_buf_model_sps_analysis, e_minFrequency);
    ...
    CLASS_ATTR_DOUBLE(c, "maxfrequency", 0, t_buf_model_sps_analysis, e_maxFrequency);
    ...
    CLASS_ATTR_LONG(c, "maxpeaks", 0, t_buf_model_sps_analysis, e_maxPeaks);
    ...
    CLASS_ATTR_LONG(c, "maxnsines", 0, t_buf_model_sps_analysis, e_maxnSines);
    ...
    CLASS_ATTR_SYM(c, "orderby", 0, t_buf_model_sps_analysis, e_orderBy);
    ...
    CLASS_ATTR_DOUBLE(c, "stocf", 0, t_buf_model_sps_analysis, e_stocf);
    ...
    CLASS_ATTR_LONG(c, "downmix",    0,    t_buf_model_sps_analysis, e_downmix);
...
void buf_model_sps_analysis_assist(t_buf_model_sps_analysis *x, void *b, long m, long a, char *s)
{
    if (m == ASSIST_INLET) {
        ...
        // @out 3 @type llll @digest Stochastic envelopes @description Stochastic envelope (one level for each channel, then one level for each frame).
        switch (a) {
            ...
            case 3: sprintf(s, "llll (%s): Stochastic envelopes", type);    break;
        }
    }
}
...
t_buf_model_sps_analysis *buf_model_sps_analysis_new(t_symbol *s, short argc, t_atom *argv)
{
    t_buf_model_sps_analysis *x;
    long true_ac = attr_args_offset(argc, argv);
    
    x = (t_buf_model_sps_analysis*)object_alloc_debug(s_model_sps_analysis_class);
    if (x) {
        ...
        earsbufobj_setup((t_earsbufobj *)x, "e", "4444", names);
...
void buf_model_sps_analysis_bang(t_buf_model_sps_analysis *x)
{
    ...
    if (num_buffers >= 1) {
        ...
        t_llll *freqs = NULL, *mags = NULL, *phases = NULL, *stocenv = NULL;
        long num_channels;
        ...
        for (long i = 0; i < num_channels; i++) {
            t_llll *channel_freqs = NULL, *channel_mags = NULL, *channel_phases = NULL, *channel_stocenv = NULL;
            t_ears_err err = ears_model_SPS_analysis((t_object *)x, samps[i], sr,
                                                      &channel_freqs, &channel_mags, &channel_phases, &channel_stocenv,
                                                      i,
                                                      &params,
        ...
        earsbufobj_outlet_llll((t_earsbufobj *)x, 3, stocenv);
