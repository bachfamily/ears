    ...
    for (long count = 0; count < num_buffers; count++, el = el && el->l_next ? el->l_next : el) {
        ...
        if (env->l_size == 0) {
            ...
            if (hatom_gettype(&env->l_head->l_hatom) == H_SYM) {
                ...
                ears_buffer_gain((t_object *)x, in, out, hatom_getdouble(&env->l_head->l_hatom), x->e_ob.l_ampunit == EARS_AMPUNIT_DECIBEL ? ears_db_to_linear : NULL);
            }
        } else {
            // gain is an envelope in llll form
            ears_buffer_gain_envelope((t_object *)x, in, out, env, x->e_ob.l_envampunit == EARS_AMPUNIT_DECIBEL ? ears_db_to_linear : NULL, earsbufobj_get_slope_mapping((t_earsbufobj *)x));
        }
