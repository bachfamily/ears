/**
	...
	@description
	Repeats portions of signal between zero crossings
	...
	@keywords
	buffer, waveset, zero-crossing, repeat
 
	@seealso
	ears.waveset.split~,ears.waveset.decimate~,  ears.waveset.subs~, ears.rubberband~, ears.soundtouch~
...
typedef struct _buf_waveset_repeat {
    t_earsbufobj       e_ob;
    long               e_span;
    long              e_normalize;
    t_llll             *args;
} t_buf_waveset_repeat;



// Prototypes
t_buf_waveset_repeat*         buf_waveset_repeat_new(t_symbol *s, short argc, t_atom *argv);
void			buf_waveset_repeat_free(t_buf_waveset_repeat *x);
void			buf_waveset_repeat_bang(t_buf_waveset_repeat *x);
void			buf_waveset_repeat_anything(t_buf_waveset_repeat *x, t_symbol *msg, long ac, t_atom *av);

void buf_waveset_repeat_assist(t_buf_waveset_repeat *x, void *b, long m, long a, char *s);
void buf_waveset_repeat_inletinfo(t_buf_waveset_repeat *x, void *b, long a, char *t);
...
EARSBUFOBJ_ADD_IO_METHODS(waveset_repeat)
...
void C74_EXPORT ext_main(void* moduleRef)
{
    ...
    CLASS_NEW_CHECK_SIZE(c, "ears.waveset.repeat~",
                         (method)buf_waveset_repeat_new,
                         (method)buf_waveset_repeat_free,
                         sizeof(t_buf_waveset_repeat),
                         (method)NULL,
                         A_GIMME,
                         0L);
    ...
    EARSBUFOBJ_DECLARE_COMMON_METHODS_HANDLETHREAD(waveset_repeat)

    // @method number @digest Set number of repetitions
    // @description A number in the second inlet sets the number of repetitions.
    ...
    CLASS_ATTR_LONG(c, "span", 0, t_buf_waveset_repeat, e_span);
    ...
    CLASS_ATTR_LONG(c, "normalize", 0, t_buf_waveset_repeat, e_normalize);
...
t_buf_waveset_repeat *buf_waveset_repeat_new(t_symbol *s, short argc, t_atom *argv)
{
    t_buf_waveset_repeat *x;
    long true_ac = attr_args_offset(argc, argv);
    
    x = (t_buf_waveset_repeat*)object_alloc_debug(s_tag_class);
    if (x) {
...
void buf_waveset_repeat_bang(t_buf_waveset_repeat *x)
{
    ...
    for (long count = 0; count < num_buffers; count++, el = el && el->l_next ? el->l_next : el) {
        ...
        ears_buffer_waveset_repeat((t_object *)x, in, out, el && is_hatom_number(&el->l_hatom) ? hatom_getlong(&el->l_hatom) : 1, x->e_span, x->e_normalize);
