...
{
    RiffWPChunkHeader RiffWPChunkHeader;
    ...
    if (wpc->reader->read_bytes (wpc->wv_in, &RiffWPChunkHeader, sizeof (RiffWPChunkHeader)) !=
        sizeof (RiffWPChunkHeader)) {
            if (error) strcpy (error, "not a valid WavPack file!");
            return WavpackCloseFile (wpc);
    }

    if (!strncmp (RiffWPChunkHeader.ckID, "RIFF", 4) && !strncmp (RiffWPChunkHeader.formType, "WAVE", 4)) {

        if (wpc->open_flags & OPEN_WRAPPER) {
            wpc->wrapper_data = (unsigned char *)malloc (wpc->wrapper_bytes = sizeof (RiffWPChunkHeader));
            memcpy (wpc->wrapper_data, &RiffWPChunkHeader, sizeof (RiffWPChunkHeader));
        }
    ...
    // the WavPack header must be first.
        ...
        while (1) {
            ...
            if (wpc->reader->read_bytes (wpc->wv_in, &WPChunkHeader, sizeof (WPChunkHeader)) !=
                ...
                if (wpc->open_flags & OPEN_WRAPPER) {
                    wpc->wrapper_data = (unsigned char *)realloc (wpc->wrapper_data, wpc->wrapper_bytes + sizeof (WPChunkHeader));
                    memcpy (wpc->wrapper_data + wpc->wrapper_bytes, &WPChunkHeader, sizeof (WPChunkHeader));
                    wpc->wrapper_bytes += sizeof (WPChunkHeader);
                }

                WavpackLittleEndianToNative (&WPChunkHeader, WPChunkHeaderFormat);
