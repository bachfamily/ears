...
void FrameCutter::compute() {
  ...
  _startIndexReal += _hopSize;
  _startIndex = (int)round(_startIndexReal); // floor
}
...
AlgorithmStatus FrameCutter::process() {
  ...
  int releaseSize = min((int)_hopSize, _frameSize); // in case hopsize > framesize
  ...
  if (acquireSize >= available) { // has to be >= in case the size of the audio fits exactly with frameSize & hopSize
    ...
    releaseSize = _startIndex >= 0 ? min(available, (int)_hopSize) : 0; // cannot release more tokens than there are available
    if (_startFromZero) {
      if (_lastFrameToEndOfFile) {
